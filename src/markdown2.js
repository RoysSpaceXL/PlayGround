      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (trimmedLine.endsWith(heading) && trimmedLine.startsWith('#')) {
          ...
          if (match && match[2].trim() === heading) {
            capturing = true;
            targetHeadingLevel = match[1].length; // Store the level of target heading
            content.push(line); // Include the heading
            continue;
          }
        }
        
        // If we hit another heading while capturing, check if we should stop
        if (capturing && trimmedLine.startsWith('#')) {
          const currentMatch = trimmedLine.match(/^(#+)/);
          if (currentMatch) {
            const currentHeadingLevel = currentMatch[1].length;
            // Only stop if we encounter a heading at same or higher level (fewer or equal # symbols)
            if (currentHeadingLevel <= targetHeadingLevel && trimmedLine !== content[0].trim()) {
              break;
            }
          }
        }
